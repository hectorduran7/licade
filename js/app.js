...
window.openMySubjectsModal = function() {
    ...
    if (!list || !window.MATERIAS_ADE) return;
    list.innerHTML = '';
    ...
    const order = ['PRIMER A�O', 'SEGUNDO A�O', 'TERCER A�O', 'CUARTO A�O', 'QUINTO A�O', 'SEXTO A�O', 'EXTRACURRICULARES'];
    order.forEach(year => {
        if (grouped[year]) {
            const title = document.createElement('div');
            title.style.cssText = "font-size:11px; font-weight:700; color:var(--text-muted); margin: 12px 0 6px 4px;";
            title.textContent = year;
            list.appendChild(title);

            grouped[year].forEach(m => {
                const isSelected = window.userMySubjects.includes(m.name);
...
window.saveMySubjects = function() {
    ...
    if (!window.currentUser || !window.db) {
        window.closeModal('mySubjectsModal');
        window.renderApp();
        return;
    }

    window.db.collection('usuarios_materias').doc(window.currentUser.uid)
        .set({ cursando: window.userMySubjects }, { merge: true })
        .then(() => {
            window.closeModal('mySubjectsModal');
            window.renderApp();
        })
